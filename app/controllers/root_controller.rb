	...
	def index
		...
		if params[:name] != 'Kanye'
			if text == 'kanye weather'
				...
				summary.downcase!
				post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "#{temp} degrees outside - #{summary}. In Kanye's words, #{message}"}.to_json
				...
				url = URI.parse('https://api.groupme.com/v3/bots/post')
				post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "Tabulating results..."}.to_json
				a = ActiveSupport::JSON.decode(post_args)
				resp, data = Net::HTTP.post_form(url, a)
				...
				if movie_title_forreal.blank?
					post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "Sorry, I couldn't find anything on the movie."}.to_json
				else
					post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "No problem. According to Rotten Tomatoes, #{movie_title_forreal} got a rating of #{movie_rating}%. #{movie_consensus}"}.to_json
				end
				a = ActiveSupport::JSON.decode(post_args)
				resp, data = Net::HTTP.post_form(url, a)
				...
				url = URI.parse('https://api.groupme.com/v3/bots/post')
				post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "Trains leaving West Dublin station for San Francisco in #{times} minutes."}.to_json
				a = ActiveSupport::JSON.decode(post_args)
				resp, data = Net::HTTP.post_form(url, a)
			elsif text == 'kanye odds of sonata emailing us about noise complaints' and params[:name] == 'Heidi Yang'
				url = URI.parse('https://api.groupme.com/v3/bots/post')
				post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "100%. Count it."}.to_json
				a = ActiveSupport::JSON.decode(post_args)
				resp, data = Net::HTTP.post_form(url, a)
				...
				xml_data.elements.each('rss/channel/item/description') do |desc| 
					if /(\d)*-(\d)*.{0,30}(victory|win)/.match(desc.text) != nil
						url = URI.parse('https://api.groupme.com/v3/bots/post')
						post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "Yes they did. #{desc.text}"}.to_json
						a = ActiveSupport::JSON.decode(post_args)
						resp, data = Net::HTTP.post_form(url, a)
						break
					elsif /(\d)*-(\d)*.{0,30}loss/.match(desc.text) != nil
						url = URI.parse('https://api.groupme.com/v3/bots/post')
						post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "No they did not. #{desc.text}"}.to_json
						a = ActiveSupport::JSON.decode(post_args)
						resp, data = Net::HTTP.post_form(url, a)
						break
					end
				...
				url = URI.parse('https://api.groupme.com/v3/bots/post')
				post_args = {"bot_id" => '1ac7158d3e349fdd6174bc11e4', "text" => "Kanye always got your back, #{first_name}."}.to_json
				a = ActiveSupport::JSON.decode(post_args)
				resp, data = Net::HTTP.post_form(url, a)
			end
		end
